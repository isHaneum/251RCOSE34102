...
Process* create_random_process(const char* id, int pid) {
    ...
    p->arrival_time     = rand() % 6;         // 도착 시간 랜덤 0~15
    p->burst_time       = rand() % 10 + 2;      // burst 시간 랜덤 2~11
    ...
    p->executed_time    = 0;                   // 실행된 순간부터 시간, 재실행시 초기화 1초 이상 실행됐는지 알기 위해해
    ...
    p -> completed_time = 0;                   // process 완료 시간(turnaound 계산용용)
